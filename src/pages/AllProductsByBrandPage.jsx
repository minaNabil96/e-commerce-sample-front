...
import { useGetProductsWithBrandQuery } from "../utils/api/apiSlice";
...
const AllProductsByBrandPage = () => {
  ...
  return (
    <Box className={`min-h-screen  `}>
      ...
      ) : isSuccess && productsWithBrand.productsByBrand.length === 0 ? (
        <LoadingsAndErrors>
        ...
        <>
          <Box className={`  grid sm:grid-cols-4 my-8`}>
            <AllProductsCard
              ...
              products={productsWithBrand.productsByBrand}
            />
          </Box>
          <Box className={`flex flex-col justify-center items-center my-8 `}>
            <PaginationComp
              pages={productsWithBrand.pagination.numOfAllPages}
